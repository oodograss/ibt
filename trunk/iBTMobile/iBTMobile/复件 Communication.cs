...
namespace iBTMobile
{
    class Communication
    {
        ...
        public BluetoothDeviceInfo[] Search()
        {
            ...
            bluetoothDeviceInfo = bluetoothClient[MAXCHANNELNUM].DiscoverDevices(12);
        ...
        */
        ...
        private void ListBoxAdd(string content)
        {
            //             if (listBox1.InvokeRequired)
            //             {
            //                 try
            //                 {
            //                     deleInvokee a1 = new deleInvokee(ListBoxAdd);
            //                     Invoke(a1, new object[] { content });//执行唤醒操作
            //                 }
            //                 catch (Exception ex)
            //                 { }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            tw.WriteStartDocument();
            tw.WriteStartElement("Bluetooth_RouteInformation");
            BluetoothDeviceInfo[] bluetoothDeviceInfo = Search();
            for (int i = 0; i < bluetoothDeviceInfo.Length; i++)
            {
                tw.WriteStartElement("Device");
                tw.WriteElementString("Name", bluetoothDeviceInfo[i].DeviceName.ToString());
                tw.WriteElementString("Address", bluetoothDeviceInfo[i].DeviceAddress.ToString());
                tw.WriteElementString("NextHopAddr", bluetoothDeviceInfo[i].DeviceAddress.ToString());
                tw.WriteEndElement();
            }
            tw.WriteEndElement();
            tw.WriteEndDocument();
            tw.Flush();
            tw.Close();
            MessageBox.Show("Save DeviceInfos Succeed!");
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            foreach (XmlElement node in DeviceList)
            {
                ...
                for (int i = 0; i < subList.Count; i++)
                {
                    if (subList[i].Name.CompareTo("Name") == 0)
                        DataRow1["DeviceName"] = subList[i].InnerText;
                    else if (subList[i].Name.CompareTo("Address") == 0)
                        DataRow1["DeviceAddress"] = subList[i].InnerText;
                }
                DataTable1.Rows.Add(DataRow1);
            }
            //             comboBox1.DataSource = DataTable1;
            ...
            MessageBox.Show("Load DeviceInfos Succeed!");
        }

        public void SendP2P(Frame fr)
        {
            fr.MsgType = "102";
            fr.SrcAddr = LocalAddress;
            fr.SeqNum = String.Format("{0:D3}", MessageSeqID);
            fr.Content = LocalName + ":" + fr.Content;
            ...
            fr.SrcAddr = LocalAddress;
            fr.SeqNum = String.Format("{0:D3}", MessageSeqID);
            fr.Content = LocalName + ":" + textBox1.Text.Trim();
            BroadcastMessage2(fr);
            MessageSeqID++;
             * */
        }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            //                 return;
            Frame fr;
            fr.MsgType = TYPE_UNICAST;
            //fr.DestAddr = comboBox1.SelectedValue.ToString();
            fr.SrcAddr = LocalAddress;
            fr.SeqNum = String.Format("{0:D3}", MessageSeqID);
            //fr.Content = LocalName + ":" + textBox1.Text.Trim();
            //             string text = EncodeMsg(fr);
        ...
        private void GetNetTopo_Click(object sender, EventArgs e)
        {
            Frame fr;
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            foreach (XmlElement node in DeviceList)
            {
                ...
                if (DataRow1["DeviceName"].ToString() != "ibt9")
                    DataTable1.Rows.Add(DataRow1);
            }
            //             comboBox1.DataSource = DataTable1;
            //             comboBox1.DisplayMember = "DeviceName";
        ...
        {
            // Connect(BluetoothAddress.Parse(comboBox1.SelectedValue.ToString()));
            /*
            if (NodeStatus == 3 || NodeStatus == 0)
                return;
            this.label1.Text = "Check in done!";
            Frame newfr = new Frame();
            newfr.MsgType = TYPE_CHECKIN;
            newfr.DestAddr = UpstreamDevice.ToString();
            newfr.SrcAddr = LocalAddress;
            newfr.SeqNum = String.Format("{0:D3}", 0);
            newfr.Content = LocalName;
            string newtext = EncodeMsg(newfr);
            SendMessage(newtext);
             * */
        }
        ...
        private void button4_Click_1(object sender, EventArgs e)
        {
            ...
            int maxquerydevicenum = 12;
            BluetoothDeviceInfo[] bluetoothDeviceInfo = { };
            bluetoothDeviceInfo = bluetoothClient[0].DiscoverDevices(maxquerydevicenum);
            for (int i = 0; i < bluetoothDeviceInfo.Length; i++)
            {
                try
                {
                ...
                JoiningQueryResult = -1;
                Joining = true;
                Frame fr;
                fr.MsgType = TYPE_JOINQUERY;
                fr.DestAddr = bluetoothDeviceInfo[i].DeviceAddress.ToString();
                fr.SrcAddr = LocalAddress;
                fr.SeqNum = String.Format("{0:D3}", 0);
                fr.Content = "";
                ...
                if (JoiningQueryResult > 0)
                {
                    bluetoothClient[JoiningQueryResult].Close();
                    bluetoothClient[JoiningQueryResult].Dispose();
                    bluetoothClient[JoiningQueryResult] = new BluetoothClient();
                    bluetoothClient[JoiningQueryResult].Connect(new BluetoothEndPoint(UpstreamDevice, ServiceName[JoiningQueryResult]));
                    BluetoothDeviceInfo deviceInfo = new BluetoothDeviceInfo(UpstreamDevice);
                    deviceInfo.DeviceName = UpstreamDeviceName;
                    UpdateRouteinfo(deviceInfo, UpstreamDevice.ToString());
                    break;
        ...
        private void button5_Click_1(object sender, EventArgs e)
        {
            Frame fr = new Frame();
            fr.MsgType = TYPE_BROADCAST;
            fr.SrcAddr = LocalName;
            fr.SeqNum = String.Format("{0:D3}", 0);
            //fr.Content = LocalName + ":" + this.textBox1.Text.Trim();
            BroadcastMessage(fr);
        }
    }
